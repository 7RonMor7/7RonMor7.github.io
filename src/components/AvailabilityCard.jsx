...
const uiText = {
  rightNow: { es: 'Ahora mismo', en: 'Right now' },
  studying: { es: 'Estudiando', en: 'Studying' },
  seeFullEducation: { es: 'Ver formación completa', en: 'See full education' },
};
    ...
    <div className="bg-slate-900/60 border border-slate-800 p-5 rounded-2xl space-y-5">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-bold text-white">{t(availability.status, language)}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-400">
          <div className="flex items-center gap-1.5">
            ...
            {t(availability.modality, language)}
          </div>
          <div className="flex items-center gap-1.5">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-blue-400 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            {t(availability.responseTime, language)}
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
          {t(uiText.rightNow, language)}
        </p>
        <p className="text-xs text-slate-300 leading-relaxed">
          {t(uiText.studying, language)}{' '}
          <span className="text-white font-medium">{t(currentFocus.studying, language)}</span>.{' '}
          {t(currentFocus.learningGoal, language)}.
        </p>
        ...
        {onSeeEducation && (
          ...
          >
            {t(uiText.seeFullEducation, language)}
